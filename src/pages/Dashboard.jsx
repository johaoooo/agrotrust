...
export default function Dashboard() {
  ...
  const quickActions = [
    { title: 'Explorer', icon: Eye, link: '/offres', gradient: 'from-emerald-500 to-teal-600' },
    { title: 'Commandes', icon: ShoppingCart, link: '/mes-commandes', gradient: 'from-blue-500 to-cyan-600' },
    { title: 'Crédit', icon: CreditCard, link: '/micro-credit', gradient: 'from-purple-500 to-pink-600' },
    { title: 'Semis', icon: Calendar, link: '/conseils-semis', gradient: 'from-orange-500 to-red-600' },
  ];

  if (authLoading || loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
        <div className="text-center">
          <div className="relative">
            <div className="animate-spin rounded-full h-16 w-16 border-b-3 border-emerald-600 mx-auto mb-4"></div>
            <Sprout className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 text-emerald-600" />
          </div>
          <p className="text-emerald-600 dark:text-emerald-400 font-medium">Chargement...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
      {/* Sidebar - FIXE, ne disparaît jamais */}
      <DashboardSidebar />
      
      {/* Contenu principal - change selon la route */}
      <div className="flex-1 overflow-x-auto">
        <div className="p-6">
          {/* En-tête - visible uniquement sur la page d'accueil du dashboard */}
          <motion.div
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
              ...
              {recentCommandes.length > 0 && (
                <button onClick={() => navigate('/mes-commandes')} className="text-xs text-emerald-600">
                  Voir tout
                </button>
              )}
            </div>
